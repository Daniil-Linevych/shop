...
const HomePage: React.FC = () => {
  ...
  const products: Product[] = [
    {
      ...
      details: 'Intel Core i5-10400F / RAM 16ТБ / SSD 240ТБ / GeForce GTX 1650 4ТБ',
      price: 1200,
      oldPrice: 1500,
      rating: 4,
  ...
  useEffect(() => {
        ...
        fakeApi.get(fakeApiEndpoints.home)
          .then((data)=>{
    ...
    <div className="bg-gray-50 min-h-screen">
        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-4">
            {categories.map((category) => (
              <CategoryItem key={category.key} icon={category.imageUrl} title={category.title} />
            ))}
          </div>
        </div>
      </section>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <section className="mb-12 text-center">
          <h2 className="text-4xl font-bold text-blue-900 mb-6">LOREM IPSUM</h2>
        ...
        <section className="mb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {brands.map((brand) => (
              <BrandCard key={brand.id} brand={brand.name} image={brand.imageUrl} />
            ))}
          </div>
        </section>
